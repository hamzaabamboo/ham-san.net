...
sampler ShadowMapSamplerAnother = sampler_state {
    texture = <ExcellentShadowZMapAnother>;
    MinFilter = LINEAR;
    MagFilter = LINEAR;
    MipFilter = LINEAR;
    AddressU  = CLAMP;
...
{
	float4 Color = float4(0,0,0,opaque);
	Color.r = tex2D( ShadowMapSampler, Tex ).r;
	Color.g = tex2D( ShadowMapSamplerFar, Tex ).r;
	Color.b = tex2D( ShadowMapSamplerAnother, Tex ).r;
	return Color;
}
